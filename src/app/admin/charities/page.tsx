...
function normalizeList<T>(data: unknown): T[] {
  ...
  if (data && typeof data === "object") {
    ...
    return record.charities ?? record.data ?? record.items ?? record.results ?? [];
  }
...
export default function CharitiesPage() {
  ...
  const fetchCharities = async () => {
    ...
    try {
      ...
      const mapped: CharityRecord[] = list.map((charity) => ({
        id:
        ...
        <div className="overflow-x-auto">
          {isLoading && (
            <div className="px-6 py-12 text-center">
              <div className="inline-block h-8 w-8 animate-spin rounded-full border-4 border-solid border-[var(--brand-teal)] border-r-transparent" />
